...
- `security_monitor(IDS).py` — Lightweight IoT IDS — monitors all MQTT topics across device channels and publishes real-time alerts
...
Card tap → JWT created
  ...
  Mosquitto Broker — authenticates device
                ↓
  Door Controller — verifies JWT → decision made
                ↓
...
interception, firmware extraction and many more. This simulation specifically only focuses on **forged JWT token injection** across door, camera, and HVAC channels targeting the device authentication layer after broker access is assumed compromised.
